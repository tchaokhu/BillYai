...
export type ReplyPlan =
  ...
  /**
   * ชื่อที่เลือกเป็นอีกแถวในบิลใบนี้อยู่แล้ว (บิล itemized เท่านั้น)
   *
   * ชื่อนั้น**ยังไม่มีเจ้าของ** ต่างจาก `name-taken` — ทางออกคือเอาชื่อนั้นออกจาก
   * บิลก่อน ไม่ใช่ไปเลือกชื่ออื่น
   */
   ...
   *
   * `eventTag` = `ยอด #เชียงใหม่` ซึ่งเป็น **สรุปของทริป ไม่ใช่ยอดค้าง** (D60) ·
   * ไม่มีคีย์นี้ = ยอดค้างทั้งวงตามเดิม
   */
  | { kind: 'balance'; eventTag?: string }
  /** มีบิลแล้วแต่ไม่มีใครติดใคร — คนละเรื่องกับวงที่ยังไม่เคยจดบิล */
  | { kind: 'settled' }
  /**
   * `ยอด #เชียงใหม่` ในวงที่มีบิลอยู่แล้ว แต่แท็กนั้นไม่มีบิลเลย (D60)
   *
   * **ไม่ยุบเป็นไกด์** — ไกด์เป็นคำตอบของวงที่ยังไม่เคยใช้ ส่วนคนนี้ใช้เป็นแล้ว
   * และพิมพ์ชื่อแท็กมาเอง สิ่งที่เขาต้องรู้คือแท็กนั้นไม่มีบิล
   */
  | { kind: 'no-bills-for-tag'; tag: string }
  /** บิลของแท็กนั้นหารกันลงตัว — ต้องพูดถึงแท็ก ไม่ใช่พูดถึงทั้งวง */
  | { kind: 'settled-for-tag'; tag: string }
  ...
  | { kind: 'bill-voided' }
  /**
   * กดปุ่ม `ยกเลิกบิล` จังหวะแรก — **ถามก่อน ยังไม่แตะอะไร** (D61)
   *
   * การ์ดรายละเอียดลอยอยู่ในแชทตลอดกาลและกดได้ทุกเมื่อ · จังหวะเดียวแล้วยอด
   * ขยับทั้งวงคือสิ่งที่จังหวะที่สองมีไว้กัน · ชื่อกับยอดอยู่ในคำถามเพราะคนกด
   * ต้องรู้ว่ากำลังจะทิ้งใบไหน
   */
  | { kind: 'confirm-void'; expenseId: string; description: string; totalSatang: number }
  /** ยกเลิกสำเร็จ — **ประกาศกลับเข้ากลุ่ม** คือ audit ของ D11 */
  | { kind: 'bill-void-done'; description: string; totalSatang: number }
  /** claim ตัวตนแล้ว แต่ไม่ใช่คนจดและไม่ใช่คนจ่ายของใบนั้น (D11) */
  | { kind: 'bill-void-not-allowed' }
  /** ยังไม่เคยยืนยันตัวตนในวงนี้ — แปลงคนกดเป็น Member ไม่ได้ (ADR 0002) */
  | { kind: 'bill-void-needs-identity' }
...
export function decideReply(context: ReplyContext, parsed: ParseResult | null): ReplyPlan {
  ...
  /**
   * `ยอด #เชียงใหม่` — คำสั่งเดียวที่รับส่วนต่อท้ายได้ (D60 ต่อจาก D34)
   *
   * **แท็กเดียวเท่านั้น** · สองแท็กแปลว่าคำถาม "ทริปไหน" ยังไม่มีคำตอบเดียว —
   * หยิบอันแรกคือเดาแทนคนถาม ส่วนรวมสองทริปเข้าด้วยกันคือตัวเลขที่ไม่มีใครขอ ·
   * ทั้งสองทางตอบผิดคำถามแบบเงียบ ซึ่งเป็นสิ่งเดียวกับที่ D34 ปฏิเสธไว้ตั้งแต่ต้น
   *
   * `parseCommand` ปล่อยผ่านเฉพาะ token ที่ขึ้นต้นด้วย `#` และยาวกว่าหนึ่งอยู่แล้ว
   * ด่านนี้จึงกันของที่ยังไม่มีในวันนี้เป็นหลัก — และกันไม่ให้ชั้นนี้พึ่งกฎที่อยู่
   * ในไฟล์อื่นเงียบๆ
   */
  if (parsed.command === 'balance' && parsed.args !== undefined) {
    const tag = parsed.args.trim()
    if (tag.startsWith('#') && tag.length > 1 && !/\s/.test(tag)) {
      return { kind: 'balance', eventTag: tag.slice(1) }
    }
    return { kind: 'not-available', what: 'command' }
  }

  // คำสั่งอื่นยังไม่มีตัวไหนรับส่วนต่อท้ายได้ (D34)
