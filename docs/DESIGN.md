...
| D38 | `expense_draft` หน้าตา | payload เป็น **`jsonb` ก้อนเดียว** · ถือ**ตัวตนฝั่ง LINE** อย่างเดียว (`line_group_id` nullable + `line_user_id` ของคนพิมพ์ `not null`) · **ไม่มี `group_id`** | ดู `docs/adr/0001-expense-draft-table.md` |
