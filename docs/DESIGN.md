...
**ของค้างของ M9 ที่รู้ตัว (2026-09-07):** การ์ด Draft ในแชท **ไม่ถูกวาดใหม่หลังเซฟจากหน้าจอ**
— LINE แก้ข้อความที่ส่งไปแล้วไม่ได้ และเอกสารระบุว่า Flex ที่ส่งด้วย `liff.sendMessages()`
ตั้งได้เฉพาะ URI action **postback ไม่ได้** ทั้งยังไม่เกิด webhook (ดู `SPIKE-PHASE0.md` §S2
หัวข้อแก้เพิ่ม) · ยอดที่ลง ledger ถูกเสมอเพราะ `confirmDraft` อ่าน `lines` ใหม่จาก DB
**แต่ตัวเลขบนการ์ดเดิมเป็นของเก่า** ซึ่งขัดหลัก "กดจากตัวเลขที่เห็น" ที่ ADR 0001 ตั้งไว้ ·
ทางที่ยังไม่ได้ตัดสิน: ให้หน้าจอส่ง **text** (ชนิดนี้เกิด webhook) เป็น Trigger ตัวใหม่ แล้วบอท
ตอบการ์ดใบใหม่ด้วย reply token ซึ่งฟรี ไม่กินโควตา 300/เดือน (C2) — ต้องตัดสินก่อนลงมือ
