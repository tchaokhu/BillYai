...
| D31 | `ยอด` หน้าตา | **Flex card ตอบในแชท · ทั้งวง · จัดกลุ่มตามเจ้าหนี้ · ไม่ตัด** | LIFF อยู่ Phase 2 ทั้งก้อน ลากเข้ามาเพื่อคำสั่งเดียวไม่คุ้ม และของที่ต้องกดเข้าไปดูทีละคนทิ้งแรงกดดันทางสังคมซึ่งเป็นสิ่งเดียวที่เรามีแทน push (C2/C3) · ทั้งวงเพราะ §3 เขียนไว้แบบนั้นและตอบได้โดยไม่ต้องรู้ว่าใครถาม ซึ่งสำคัญเพราะ D29 ถามตัวตนบนการ์ด Draft เท่านั้น คนที่ยังไม่เคยจดบิลจึงยังไม่มี Member · จัดกลุ่มตามเจ้าหนี้เพราะคนพิมพ์ `ยอด` คือคนที่ควักเงินไปก่อน เขาหาชื่อตัวเองในหัวบล็อกแล้วอ่านจบในที่เดียว · ไม่ตัดเพราะ Phase 1 ไม่มี LIFF ให้ไปดูส่วนที่ถูกตัด และ ledger ที่ซ่อนยอดโดยไม่มีทางเปิดดูรับไม่ได้ · **แก้ข้อเท็จจริงตอนลงมือ M7: เพดาน Flex เป็นตัวบังคับจริง** — แถวหนึ่งแถวหนักราว 250 ไบต์เพราะเป็น box ที่มี text สองก้อน วง 8 คนที่ทุกคนเคยจ่าย (28 คู่) ทะลุ 10 KB ไปแล้ว · ตัวเลข "28 แถวราว 3 KB" ที่เขียนไว้ตอนตัดสินเป็นการเดา ไม่ได้วัด · ทางลงคือ **ลดรูปเป็นข้อความ ไม่ใช่ตัดคนออก** (D44) |
| D44 | การ์ด `ยอด` ที่ใหญ่เกิน bubble | **ลดรูปเป็น text message ไม่ตัดใครทิ้ง** · เกิน 5 ก้อนค่อยบอกตรงๆ ว่าตัด | ทะลุ 10 KB แล้ว LINE ปฏิเสธทั้งข้อความ คนพิมพ์ `ยอด` จะไม่เห็นอะไรเลย ซึ่งแย่กว่าการ์ดหน้าตาไม่สวย · D31 ห้ามตัดคนออกเพราะ Phase 1 ไม่มีที่ให้ไปดูส่วนที่ถูกตัด — การเปลี่ยนรูปแบบจึงเป็นทางลงเดียวที่ไม่ขัดข้อนั้น · reply ส่งได้ 5 ก้อน ก้อนละ 5000 ตัวอักษร รับได้ราว 800 แถว ซึ่งมากกว่าวงจริงทุกขนาด · เกินกว่านั้นบอกว่ามีต่ออีกกี่ส่วน ไม่ใช่เงียบๆ ตัดทิ้ง ซึ่งใน ledger คือยอดหาย |
...
| D41 | `+ ข้าว 1200` ในวงที่ Roster ว่าง | **ขอชื่อ ไม่สร้าง draft** | ไวยากรณ์แปลว่า "หารเท่าทุกคนใน Roster" — Roster ว่างจึงไม่มีใครให้หาร · การ์ดที่มีแต่คนจ่ายคนเดียวคือบิลที่ไม่มีหนี้อยู่ในนั้น กดยืนยันแล้วไม่ได้อะไร แถมยังทิ้งแถวไว้ · เงียบก็ไม่ได้เพราะเขาพิมพ์ถูกไวยากรณ์ทุกอย่าง · คำตอบยกตัวอย่างไวยากรณ์ที่มีชื่อคนมาให้ด้วย ไม่ใช่บอกว่าไม่รู้จักแล้วจบ |
| D42 | LINE ไม่ส่ง `userId` มา | **บอกตรงๆ ว่าจดให้ไม่ได้และบอกสาเหตุ** | เกิดเมื่อคนพิมพ์ยังไม่ยอมรับข้อตกลงการใช้งานบัญชีทางการของ LINE · D26 ให้เฉพาะคนพิมพ์กดยืนยันได้ ซึ่งเช็คไม่ได้เลยถ้าไม่รู้ว่าใครพิมพ์ จึงสร้าง draft ไม่ได้ตั้งแต่ต้น · "ลองใหม่อีกครั้ง" เฉยๆ ไม่ช่วย เพราะไม่มีอะไรเปลี่ยนจนกว่าเขาจะไปกดยอมรับ — ข้อความจึงต้องบอกสาเหตุจริง |
| D43 | ไม่ระบุชื่อใครเลย หารกับใคร | **Roster + คนจ่าย** — คนที่ยืนยันตัวตนแล้วอยู่ใน Roster อยู่แล้วจึงไม่ได้แถวซ้ำ | เดิม M5 หารเฉพาะ Roster ทำให้คนพิมพ์ที่ยังไม่ claim ไม่โดนอะไรเลยทั้งที่เขาก็กินด้วย · และทำให้ `รวมฉัน` กลายเป็นคำที่ไม่มีผล เพราะ parser ยุบ `+ ข้าว 1200` กับ `+ ข้าว 1200 รวมฉัน` เป็น draft เดียวกันเป๊ะ (`participants: []`, `includesPayer: true`) — คนที่พิมพ์ `รวมฉัน` แล้วยังไม่ claim ถูกตัดออกเงียบๆ ทั้งที่ขอเข้าเอง · **แก้ที่ความหมายแทนที่จะเพิ่มฟิลด์ใน `ParseResult`**: ทั้งสองแบบรวมคนจ่ายเหมือนกัน `รวมฉัน` จึงซ้ำซ้อนแต่ไม่ผิด · ระบุชื่อคนแล้วยังเหมือนเดิม คนจ่ายไม่ร่วมหารเว้นแต่พิมพ์ `รวมฉัน` |
| D45 | ไล่ไม่ได้ว่ายอดมาจากบิลใบไหน | **คำสั่ง `บิล` แยกจาก `ยอด`** — 20 ใบล่าสุดเป็นแถวชื่อ+วันที่+ยอด กดแถวเพื่อดูรายละเอียดรายคน | `ยอด` ไม่ยุบข้ามคน (D5) แต่**ยุบข้ามบิล**ของคู่เดียวกัน — เดียร์ติดนัท 900 คือสามใบรวมร่างเป็นเลขเดียว ไม่มีทางย้อนดู · กางรายละเอียดทุกใบในการ์ดเดียวไม่ได้ เพราะแถวโตตามขนาดวง วง 4 คนแค่ 8 ใบก็ทะลุ 10 KB · แถวชื่ออย่างเดียวหนักคงที่ราว 250 ไบต์ ไม่ขึ้นกับจำนวนคน ใส่ได้ราว 35 ใบ · กดแถวส่ง postback ที่พา `expense.id` (uuid 36 ตัว) **ไม่ใช่ชื่อ** — ชื่อไทยผ่าน `encodeURIComponent` ทะลุเพดาน 300 ตั้งแต่ราว 27 ตัว (ADR 0002) · **เป็นข้อเท็จจริงในอดีตล้วน ไม่บอกว่าใบไหนยังค้าง** เพราะ `settlement` ไม่ได้ชี้ `expense` คำว่า "บิลใบนี้ยังไม่ถูกจ่าย" ไม่มีอยู่ในระบบ (D33) — การ์ดที่อ้างแบบนั้นคือตัวเลขผิดใน ledger ซึ่งแย่กว่าดูยาก · บิลที่ `status='voided'` ตอบว่าถูกยกเลิกแล้ว ไม่ใช่เงียบและไม่ใช่โชว์ยอดเก่า เพราะการ์ดเก่าลอยอยู่ในแชทและกดได้เสมอ (D30 ไม่มี hard delete) |
| D46 | `บิล` ที่แสดงไม่ครบ | **ปุ่ม `ดูทั้งหมด` เปิด LIFF** — ลงมือได้เมื่อ Phase 2 มี LIFF แล้ว | D31 ห้ามตัดด้วยเหตุผลเดียวคือ "Phase 1 ไม่มี LIFF ให้ไปดูส่วนที่ถูกตัด" — มี LIFF เมื่อไหร่เหตุผลนั้นหมดอายุ **ตัดได้โดยไม่ต้องแก้ D31** · ระหว่างที่ยังไม่มี ท้ายการ์ดเขียนจำนวนที่เหลือแทน ไม่ตัดเงียบ (D44) · สิทธิ์ดูมาจาก DB เราเท่านั้นตาม D15 — `liff.getContext().groupId` กับ query string มาจากฝั่ง client **เชื่อไม่ได้** มันบอกว่าขอดูวงไหน ไม่ใช่ว่ามีสิทธิ์ดู · ลิงก์พา `ledger_group.id` (uuid) **ไม่ใช่ `line_group_id`** ซึ่งเป็น id จริงของ LINE และ repo เป็น public · ผลข้างเคียงที่ยอมรับ: คนในกลุ่มที่ยังไม่เคย claim ไม่มี Member จึงเปิดไม่ได้ — หน้า LIFF บอกให้กดยืนยันบิลสักใบก่อน ซึ่งตรงกับ D15 โดยไม่ต้องเพิ่มกลไก · LIFF ต้องอยู่ **provider เดียวกับ Messaging API channel** ไม่งั้น `userId` เป็นคนละตัวแล้ว D4/D10 พังเงียบไม่มี error ฟ้อง |
| D47 | คำสั่งคีย์เวิร์ดในกลุ่ม | **ต้อง @mention บอทเสมอ** — `@บิลใหญ่ ยอด` · `@บิลใหญ่ บิล` · คำสั่งอื่นทุกตัวในอนาคตด้วย · **`+` จดบิลไม่ต้อง** · **แชท 1:1 ไม่ต้อง** · คีย์เวิร์ดเปล่าๆ ในกลุ่ม = เงียบ | **แคบกว่าที่ §3 ตัดสินไว้ 2026-08-29 ไม่ได้ล้มมัน** — ย่อหน้านั้นเถียงเรื่อง `+` ซึ่งเป็นการกระทำที่ถี่ที่สุดในระบบและยังไม่ถูกแตะ · คำสั่งคีย์เวิร์ดเป็นคนละเรื่อง: `ยอด` กับ `บิล` เป็นคำที่คนในกลุ่มพูดกันเองจริง (`ยอด มาไหม` · `บิลเท่าไหร่`) การชนจึงไม่ใช่ edge case แต่เป็นกรณีปกติ · **ได้กฎที่หายไปทั้งคลาส**: D34 เคยต้องแก้เฉพาะ `ยอด` ด้วยเงื่อนไข "ส่วนต่อท้ายต้องเป็น `#tag` ล้วน" — บังคับ mention ทำให้ไม่ต้องคิดเรื่องชนคำให้คำสั่งใหม่ทีละตัวอีก · 1:1 ยกเว้นเพราะ LINE ไม่มี mention ที่นั่น บังคับไม่ได้เชิงเทคนิค และเป็นเกณฑ์เดียวกับกฎเงียบซึ่งเป็นของกลุ่มอย่างเดียวอยู่แล้ว (`lib/flow/dispatch.ts`) · **คีย์เวิร์ดเปล่าๆ ในกลุ่มต้องเงียบ ไม่ใช่ตอบว่าให้ใส่ `@`** เพราะคำตอบแบบนั้นจะเด้งใส่ `ยอด มาไหม` พอดี ซึ่งคือสิ่งเดียวที่กฎเงียบมีไว้กัน · ราคาที่จ่าย: คนที่เคยพิมพ์ `ยอด` เปล่าๆ แล้วได้ผลจะเจอความเงียบซึ่งอ่านออกได้ว่าบอทพัง — **จึงเปลี่ยนตอนนี้ ตอนที่ยังไม่มีผู้ใช้นอกกลุ่มทดสอบ** ราคานี้ขึ้นทุกวันหลังจากนี้ |
| D48 | ข้อมูลรายชิ้นเข้าทางไหน | **หน้าจอ LIFF ติ๊กชื่อคนกิน ไม่ใช่ไวยากรณ์ในแชท** — ยืนตาม §3 เดิม | ตัดสินให้กลับคำเป็นไวยากรณ์ `-` ในแชทเมื่อ 2026-09-05 แล้ว**กลับมาวันเดียวกันก่อนเขียนโค้ด** · ราคาที่แพงไม่ใช่ตัวไวยากรณ์ แต่คือการพิมพ์ชื่อคนกินต่อชิ้นด้วยนิ้วโป้งหน้าเคาน์เตอร์ ซึ่งชนกับ D19 ตรงๆ · ทางลดการพิมพ์ทุกแบบ (เลขประจำคน · ชุดคนกินนำหน้า · ระบุคนที่ไม่กิน) ชนะเฉพาะบางรูปใบเสร็จ ไม่ชนะรูปที่เป็นต้นเรื่อง · ปุ่มติ๊กในแชททำไม่ได้เชิงเทคนิค LINE ไม่มีการแก้ข้อความที่ส่งไปแล้ว — ดู `docs/adr/0003` (กลับคำแล้ว) |
| D49 | LIFF มาเมื่อไหร่ | **ขยับขึ้นมาก่อนการทวง** — งานแรกของ LIFF คือหน้าจอ itemized ไม่ใช่ Nudge | บิลที่จดไม่ได้ในคืนนั้น**หายถาวร** ไม่มีใครกลับไปนึกว่าใครกินอะไรเมื่อสองสัปดาห์ก่อน · หนี้ที่ยังทวงไม่ได้ยังอยู่ครบใน ledger และก๊อปยอดจากการ์ด `ยอด` ไปทวงมือได้วันนี้ · LIFF init + verify ID token (D15) เป็นฐานร่วมที่ Nudge ต้องใช้อยู่แล้ว ทำก่อนไม่ได้ทิ้งงาน — ดู `docs/adr/0005` |
| D50 | ตัวเลขท้ายบิลเก็บยังไง | **`adjustment_satang` — จำนวนเงินสตางค์ ไม่ใช่เปอร์เซ็นต์** · ปกติคำนวณจาก ยอดที่จ่ายจริง − ผลรวมรายชิ้น | `numeric(5,2)` เก็บ `4700/44000 = 10.6818…%` ไม่ลงตัว แล้ว ledger จะได้ 486.99 ทั้งที่ใบเสร็จเขียน 487.00 ซึ่งขัด invariant ใน `CONTEXT.md` · เกิดกับทุก surface ที่มี Item ไม่ใช่เฉพาะแชท · **migration ไปพร้อม M9 ไม่ใช่ตอนนี้ และต้องรันทั้งสอง Supabase project** — ดู `docs/adr/0004` |
| D54 | ส่วนปรับตั้งเองได้ไหม | **ได้ สองทาง — เป็น % หรือเป็นบาท — แต่ไม่เปลี่ยนยอดที่จ่ายจริง มันเป็นตัวทานชั้นที่สอง** · ตั้งเอง = ติดลบได้ (ส่วนลด) · คำนวณเอง = ติดลบไม่ได้ · **% ไม่ได้เก็บลง DB** เป็นเครื่องคิดเลขบนจอที่ให้จำนวนเงินสตางค์ออกมา | สามตัวเลขผูกกันด้วย `ยอดที่จ่ายจริง = ผลรวม Item + ส่วนปรับ` แก้ได้แค่สองตัว · ให้ส่วนปรับเป็นตัวตั้งแล้วยอดที่จ่ายจริงเป็นตัวคำนวณ = **ตัวทานหายทั้งตัว** ลืมจดรายการแล้วบิลลง ledger น้อยกว่าใบเสร็จเงียบๆ · ที่เลือกคือยอดที่จ่ายจริงเป็นตัวตั้งเสมอ ค่าที่พิมพ์เองไปเทียบ `ยอด − ส่วนปรับ` กับผลรวม Item จริง ไม่ตรง = เตือน · ส่วนลดกับคูปองเป็นของจริง แต่ค่าติดลบที่ระบบคำนวณเองแยกไม่ออกจากการจดตก **เกณฑ์คือมีคนตั้งใจหรือไม่ ไม่ใช่เครื่องหมายของตัวเลข** · **สามโหมด**: `auto` ส่วนต่าง · `pct` `รวมรายชิ้น × %` ขยับตามเมื่อรายการเปลี่ยน · `amount` เลขที่พิมพ์ตายตัว — สองโหมดหลังทำให้ยอดหัวกลายเป็นตัวทาน ลืมจดรายการแล้วผลบวกไม่ถึงยอดหัว จึงยังจับได้ · หน้าจอเป็น **ติ๊กสองช่อง service charge / VAT** ค่าเริ่มต้น 10% กับ 7% กรอกทับได้ — **VAT ทบบน `subtotal + service` ไม่ใช่บน subtotal เปล่าๆ** (10% แล้ว 7% = 17.7% ไม่ใช่ 17%) กฎการทบนี้ต้องมีเทสต์คุม · ติ๊กออกหมด = กลับโหมด auto ไม่มีสถานะที่สี่ · **ห้ามเติมยอดหัวให้อัตโนมัติ** ต่อให้คำนวณได้ ปุ่มนั้นคือปุ่มที่ทำให้รายการที่ลืมจดหายเงียบ |
| D51 | การ์ดของบิล itemized ในแชท | การ์ดรายละเอียด (`บิล` → กดแถว) โชว์ **รายการ + รายคน** ทั้งสองส่วน · **ไม่แยกการ์ดสองแบบ** | รายการตอบ "ใครกินอะไร" รายคนตอบ "ฉันติดเท่าไหร่" เป็นคำถามคนละข้อ · บิลที่ไม่มี Item ไม่ได้ขาดข้อมูล ส่วนรายการจึงหายไปเฉยๆ **ห้ามเขียนว่า "ไม่มีรายการ"** เพราะบิลที่จดแบบยุบตามชุดคนกินจะไม่มีวันมี Item |
| D52 | การ์ดยาวเกิน bubble | **carousel** · **ห้ามตัดเป็นบิลใบที่สอง** · `draftCardMessage` ต้องมีทางลงด้วย | ตัดสองบิลทำให้ยอดหัวของแต่ละใบเป็นตัวเลขที่ไม่มีใครพิมพ์ และถ้ายืนยันสองครั้งแล้วกดใบเดียว **ครึ่งบิลลง ledger** · D44 ยอมลดรูปได้เพราะเป็นแค่การแสดงผล ข้อนี้ไม่ใช่ · **ลงมือแล้ว 2026-09-06**: `balanceCardMessage` `billDetailCardMessage` `draftCardMessage` ได้บันไดเดียวกัน — bubble เดี่ยว → carousel → text · เพดานตั้งไว้ **10 bubble / 45 KB** ซึ่งปลอดภัยใต้ตัวเลขที่เข้มที่สุดที่หาเจอ (เอกสาร LINE ห้าหน้าที่ยิงดูไม่บอกตัวเลขตรงๆ) · เดาต่ำแล้วตกเป็น text เร็วไปคือพฤติกรรมเดิมอยู่แล้ว จึงไม่มีทางแย่ลงกว่าวันนี้ · **ยังต้องยิงของจริงยืนยันว่า carousel ขึ้นจริงในแชท** · **ครบแล้ว 2026-09-09 (D59)** — ขั้นสุดท้ายของ `draftCardMessage` เคยคืน bubble ใบเกินเพดาน ตอนนี้ลดรูปเป็นข้อความที่ยังกดยืนยันได้ |
| D55 | เพิ่มคนหารจากหน้าจอยังไง | **สองชั้น** — `คนในบิล` เริ่มจากชื่อที่พิมพ์ในแชท เพิ่มได้จาก Roster หรือพิมพ์ชื่อใหม่ · ชิปต่อรายการเลือกได้เฉพาะคนที่อยู่ในบิลแล้ว · ชื่อใหม่ที่**คล้ายของเดิม เตือน ไม่ห้าม** · เอาคนออกที่เป็นคนกินคนเดียวของรายการไหน **ถามก่อน** · **คนจ่ายเอาออกไม่ได้** · **บิลที่คนจ่ายไม่ได้กินด้วยไม่มีแถวของเขาในบิลเลย** — ไม่มีอะไรให้ล็อก และห้ามเดาว่าคนแรกในลิสต์คือคนจ่าย | ดึงรายชื่อสมาชิกกลุ่มจาก LINE ไม่ได้ (C1) คนใหม่จึงต้องมีคนพิมพ์ชื่อ — แต่พิมพ์ครั้งเดียวต่อคน ไม่ใช่ต่อรายการ ซึ่งคือราคาที่ทำให้ ADR 0003 ถูกกลับคำ · ดึง Roster ทั้งวงมาเป็นชิปใต้ทุกบรรทัดแปลว่าทริป 15 คนได้ชิป 15 อันคูณจำนวนรายการ · `unique (group_id, display_name)` + D18 ห้ามลบ = **ชื่อที่พิมพ์ผิดจองสลอตถาวร** (`0001_init.sql:71` เตือนไว้เอง) แต่ห้ามสร้างไม่ได้เพราะ `โจ้` กับ `โจ` เป็นคนละคนได้จริง · เอาคนออกแล้วรายการที่เหลือคนกินศูนย์กลายเป็นของกลางตาม D53 ซึ่งขยับยอดรายคนของทุกคน **เงียบๆ** จึงต้องบอกชื่อรายการก่อน |
| D53 | รายการที่ไม่ระบุชื่อใคร | **ของกลาง หารกับทุกคนในบิล** | ข้าวเปล่า น้ำแข็ง ของกลางคือกรณีปกติของโต๊ะอาหาร ไม่ใช่ edge case · ด่าน `memberIds` ว่าง = error ใน `split.ts:151` ยังอยู่ หน้าจอเป็นคนเติมชื่อให้ครบก่อนส่งเข้าไป |
| D56 | หน้าจอ LIFF เปิดจากไหน | **ปุ่มบนการ์ด Draft อย่างเดียว** — พา `draftId` ไปด้วย · ไม่มีคำสั่งเปิดลอยๆ ในเฟสนี้ | `expense_draft` เก็บ `line_user_id` ของคนพิมพ์อยู่แล้ว และ ID token คืน `sub` เป็นค่าเดียวกัน — ด่านสิทธิ์จึงเป็นการเทียบกับแถวที่เราเขียนเอง **ไม่ต้องเชื่ออะไรที่ client ส่งมาเลย** ซึ่งตรงกับ D46 ที่บอกว่า `liff.getContext().groupId` บอกได้แค่ว่าขอดูวงไหน ไม่ใช่ว่ามีสิทธิ์ · เปิดลอยๆ ชนกำแพง ADR 0002 ตรงๆ: คนที่ยังไม่ claim ตัวตนแปลง `sub` เป็น `MemberId` ไม่ได้ และกลไก claim วันนี้อยู่บน quick reply ในแชท การเปิดลอยๆ จึงต้องสร้าง claim flow ชุดที่สองบนเว็บก่อน · `แก้` (§3) ยังเป็นทางเปิดในอนาคต ไม่ได้ถูกตัด |
| D57 | หน้าจอ LIFF เขียน DB ตอนไหน | **เซฟกลับลง `expense_draft` เท่านั้น · ปุ่มยืนยันยังอยู่ในแชท** | D30 ให้เขียน ledger ตอนกดยืนยันที่เดียว และ draft ไม่ใช่ ledger การเซฟระหว่างแก้จึงไม่ขัด · ให้หน้าจอกดยืนยันแล้วเขียน ledger เองน้อยขั้นกว่าหนึ่งจังหวะ แต่เปิดเส้นทางเขียน ledger เส้นที่สอง (ต้องคุมเทสต์ซ้ำทั้งชุด) และการ์ดผลลัพธ์ต้องไป **push** เข้ากลุ่มซึ่งกินโควตา 300/เดือน (C2) ส่วนทางที่เลือกใช้ reply ที่มีอยู่แล้ว · ผลพลอยได้ที่สำคัญกว่า: กลุ่มได้เห็นบิลก่อนมันลง ledger เหมือนเดิมทุกใบ ไม่ว่าจดมาทางไหน · autosave ทุกคีย์ถูกปฏิเสธ — ทนเน็ตหลุดดีกว่าจริง แต่ต้องตัดสินเรื่อง conflict ตอนสองคนแก้ใบเดียวกัน ซึ่งยังไม่เคยตัดสิน |
| D58 | การ์ดในแชทค้างตัวเลขเก่าหลังเซฟจากหน้าจอ | **หน้าจอยิง text พร้อมลิงก์ LIFF ของตัวเองกลับเข้าแชท แล้วบอทตอบการ์ดใบใหม่ด้วย reply token** — ตัวอ่านอยู่ที่ `lib/liff/echo.ts` ขอบเขตอยู่ที่ `findDraftInScope` (วง ไม่ใช่เจ้าของ) · ส่งไม่ได้ = ข้อความท้ายหน้าจอเปลี่ยนไปบอกว่ายอดชุดใหม่ลง ledger อยู่ดี ไม่ใช่ error | `liff.sendMessages()` ส่ง Flex ได้ แต่เอกสาร LINE ระบุสองข้อที่ฆ่าแผน "หน้าจอยิงการ์ดใบใหม่เอง": Flex/template ที่ส่งด้วยวิธีนี้ตั้งได้เฉพาะ **URI action** (ปุ่มยืนยันเป็น postback) และ **ไม่เกิด webhook เลย** ส่วน text เกิดตามปกติ · reply token ฟรี ไม่กินโควตา push 300/เดือน (C2) · **ยกเว้น D47 ได้เพราะลิงก์ไปหา LIFF ของบอทเราเองคือการเรียกบอท** ไม่ใช่คำที่คนในกลุ่มบังเอิญพูดตรงกัน ซึ่งเป็นคลาสเดียวที่ D47 มาปิด — และข้อความที่ส่งผ่าน API สร้าง mention ไม่ได้ บังคับ mention ตรงนี้จึงแปลว่าทำไม่ได้เลย · ขอบเขตเป็น **วง** ไม่ใช่เจ้าของ ต่างจาก `authorizeDraft` (D26) เพราะเส้นนี้แค่วาดการ์ดที่ลอยอยู่ในกลุ่มให้ทุกคนเห็นอยู่แล้วซ้ำ ไม่ได้แก้อะไร · การ์ดวาดจาก**มุมของเจ้าของ draft** ไม่ใช่คนแปะลิงก์ ไม่งั้นแถวเลือกตัวตน (ADR 0002) จะไปถามผิดคน · ราคา: กลุ่มเห็นข้อความจากผู้ใช้เพิ่มสองบรรทัด และการ์ดใบเก่ายังลอยอยู่ (กดแล้วยังลงยอดชุดใหม่ถูกต้อง เพราะ `confirmDraft` อ่าน lines จาก DB) · ต้องเปิด scope `chat_message.write` ที่ LIFF app — ดู `SETUP-DEPLOY.md` §4.3 |
| D59 | การ์ด Draft ที่ใหญ่เกิน carousel | **ลดรูปเป็นข้อความที่ยังกดยืนยันได้ผ่าน quick reply** — ชื่อทุกคนอยู่ครบ · เกินห้าก้อนต่อ reply แล้วบอกตรงๆ ว่าตัด | เดิมบรรทัดสุดท้ายของ `draftCardMessage` คืน bubble ใบเกินเพดานออกไป (`pages === null` หรือ carousel เกิน 45 KB) · LINE ปฏิเสธ reply ทั้งก้อน แล้ว **แถว draft ถูกเขียนไปแล้วแต่ไม่มีการ์ดให้ใครกด** กู้ไม่ได้ 24 ชม. และพิมพ์ใหม่ก็ได้ผลเดิม — เป็นทางตันที่ไม่มีใครได้คำตอบสักครั้ง · **postback ติดกับ text message ได้ผ่าน quick reply** ซึ่งเป็นสิ่งเดียวที่ทำให้ข้อความนี้ยังเป็นการ์ดได้ · quick reply เกาะ**ก้อนสุดท้าย**เท่านั้น เพราะ LINE แสดงของก้อนสุดท้าย · ปุ่มจดรายชิ้นเป็น URI ซึ่ง quick reply ของเราไม่รับ ลิงก์จึงอยู่ในเนื้อข้อความ · เกณฑ์ **ลดรูป ไม่ใช่ตัดเนื้อหา** ยกมาจาก `balanceCardMessage` ตรงๆ (D16 ห้ามให้ชื่อใครหายจากบิลเงียบๆ) · วัดแล้วรูปแบบเปลี่ยนที่ราว 350 คนในบิลเดียว ซึ่งไปไม่ถึงจริง — แต่ราคาของการไม่มีทางลงคือทางตัน ส่วนราคาของการมีคือฟังก์ชันคืน array ซึ่ง `balanceCardMessage` ทำอยู่แล้ว |
| D60 | `ยอด #tag` ตอบอะไร | **สรุปของบิลที่ติดแท็กนั้น ไม่ใช่ยอดค้าง** — หัวการ์ด `สรุป #เชียงใหม่` · หัวบล็อกใช้ `ออกไปก่อน` ไม่ใช่ `ได้คืน` · **ไม่มีคำว่า "ค้าง" บนการ์ดใบนั้นเลย** · แท็กเดียวเท่านั้น · แท็กที่ไม่มีบิลตอบต่างจากวงที่ไม่มีบิล | `settlement` ไม่มี `event_tag` และไม่ชี้ `expense` (D33 ตั้งใจ) — **เงินที่จ่ายคืนกันแล้วหักออกจากทริปไหนก็ผิดทั้งนั้น** จึงคิดจากบิลอย่างเดียวและส่ง `settlements: []` เข้า `computeDebts` โดยรู้ตัว · D34 ปฏิเสธ "ตอบยอดพร้อม disclaimer" ไว้แล้วเพราะการ์ดจะมีสองความหมายในใบเดียว และคนที่กวาดตาผ่านจำแค่ตัวเลข — **ทางออกจึงเป็นการเปลี่ยนคำทั้งชุด ไม่ใช่เติมบรรทัดเตือน** บรรทัดเตือนยังมีอยู่แต่เป็นของแถม ไม่ใช่สิ่งเดียวที่กันความเข้าใจผิด · สองแท็กยังไม่เปิด: หยิบอันแรกคือเดาแทนคนถาม รวมสองทริปคือตัวเลขที่ไม่มีใครขอ ทั้งคู่คือการตอบผิดคำถามแบบเงียบซึ่งเป็นสิ่งที่ D34 ปฏิเสธตั้งแต่ต้น · `no-bills-for-tag` ไม่ยุบเป็น `no-bills` เพราะ `no-bills` พาไปตอบไกด์ ซึ่งเป็นคำตอบของวงที่ยังไม่เคยใช้ ส่วนคนนี้ใช้เป็นแล้วและพิมพ์ชื่อแท็กมาเอง สิ่งที่เขาต้องรู้คือพิมพ์ชื่อไม่ตรง · **ทางที่ปฏิเสธ**: ผูก `event_tag` เข้ากับ `settlement` (migration 0004) เพื่อให้เป็นยอดค้างจริง — ตัดสินโมเดลหนี้ล่วงหน้าทั้งที่ยังไม่มี surface ไหนสร้าง settlement เลย จึงทดสอบจริงไม่ได้ในวันนี้ · วันที่ Phase 2 ลง เส้นนี้ต้องถูกทบทวนอีกครั้ง — ตอนนี้ยังไม่มี settlement ในระบบ ตัวเลขจึงตรงกับยอดค้างพอดีโดยบังเอิญ |
| D61 | ยกเลิกบิลยังไง | **ปุ่ม `ยกเลิกบิล` บนการ์ดรายละเอียด · สองจังหวะ · สิทธิ์ตาม D11 (คนจด หรือคนจ่าย) · ประกาศกลับเข้ากลุ่มเสมอ** | ทางเข้าใช้เส้น D45 ที่มีอยู่แล้วทั้งเส้น (`บิล` → กดแถว → การ์ด) จึงผูกกับบิลใบเจาะจงโดยไม่ต้องพิมพ์ id และไม่ต้องนิยามว่า "คำสั่งล่าสุด" คืออะไรแบบที่ `เลิก` ใน §3 ต้องทำ · **สองจังหวะ** (`void=<id>` ถาม → `void=<id>&yes=1` ลงมือ) เพราะการ์ดรายละเอียดลอยอยู่ในแชทตลอดกาลและกดได้ทุกเมื่อ — จังหวะเดียวแล้วยอดของทุกคนในวงขยับคือสิ่งที่จังหวะสองมีไว้กัน · คำถามถือชื่อกับยอดของบิลไปด้วย เพราะคนกดต้องรู้ว่ากำลังจะทิ้งใบไหน · **สิทธิ์ไม่ได้อยู่ที่ปุ่ม** — การ์ดใบเดียวถูกส่งเข้ากลุ่มให้ทุกคนเห็นเหมือนกันหมด ซ่อนตามคนดูทำไม่ได้ (เกณฑ์เดียวกับ D56) ด่านจึงอยู่ที่ `voidBill` ฝั่ง repo · D11 บอกว่า "คนจด + Payer แก้ได้" — เอาทั้งสองขา ไม่ยุบเหลือคนจดแบบ D26 เพราะคนที่ควักเงินไปก่อนคือคนที่รู้ดีที่สุดว่าบิลใบนั้นผิด · คนที่ยังไม่ claim ตัวตนยกเลิกไม่ได้ (ADR 0002 แปลง `line_user_id` เป็น Member ไม่ได้) ซึ่งเป็นราคาที่รับได้เพราะเขายืนยันบิลของตัวเองก็ยังไม่ได้เหมือนกัน · **ไม่ลบแถว** `voidExpense` มาร์ก `status` เท่านั้น — `computeDebts` ข้าม voided ให้เอง และ `listExpenses`/`countExpenses` กรอง `active` อยู่แล้ว ยอดกับรายการจึงขยับเองทั้งคู่ · **ประกาศกลับเข้ากลุ่มคือ audit ของ D11** ไม่ใช่ตาราง log — `voidBill` จึงคืนชื่อกับยอดมาด้วย ไม่ใช่คืนแค่ว่าสำเร็จ · **ทางที่ปฏิเสธ**: (ก) คำสั่ง `เลิก` ตาม §3 — ต้องเก็บ state ว่าใครทำอะไรไปล่าสุด ซึ่งเป็นงานใหญ่กว่าการยกเลิกบิลเอง (ข) ให้ใครในวงก็ยกเลิกได้ตามเหตุผลของ D11 ("audit = สังคมคุมแทน permission") — เปิดทางให้ลบบิลของคนอื่นได้ ซึ่งกู้ไม่ได้เพราะยังไม่มีคำสั่ง restore · **คนไม่มีสิทธิ์ได้คำตอบ ไม่ใช่ความเงียบ** ต่างจาก `not-yours` ของ D26 ที่เงียบเพื่อไม่ให้คนกดผิดเสียหน้า — ที่นี่เขากดมาสองจังหวะแล้ว และ `displayText` ของปุ่มพาคำว่า `ยกเลิกบิล` ขึ้นในแชทในนามเขาไปเรียบร้อย ความเงียบหลังจากนั้นอ่านออกได้อย่างเดียวว่าบอทพัง · **`voidBill` มีด่าน `canReply` ที่ route เหมือน `saveDraft`** — ยกเลิกสำเร็จแล้วประกาศไม่ออกคือกรณีที่แย่ที่สุดของฟีเจอร์นี้: ยอดทั้งวงขยับโดยไม่มีใครรู้ ทั้งที่การประกาศคือ audit ตัวเดียวที่มี |

### การตัดสินใจที่ตกลงระหว่างทาง (ไม่ได้อยู่ใน 19 ข้อ)

- **เก็บทุกจำนวนเป็นสตางค์ (integer)** — ไม่มี float ที่ไหน. เศษจากการหารใช้
  largest-remainder แล้ว Payer รับเศษ. invariant: `Σ share = total + adjustment`
- **Adjustment หนึ่งตัวต่อบิล** — บิลร้านไทยคือ `รายการ → +10% → +7%` ถ้า itemized
  ไม่มีตัวนี้ ระบบพังทันทีในวันแรก · **เก็บเป็นจำนวนเงินสตางค์ ไม่ใช่เปอร์เซ็นต์**
  · ปกติไม่มีใครพิมพ์มันเข้ามา มันคือส่วนต่างระหว่างยอดที่พิมพ์กับผลรวมรายชิ้น
  แต่ **พิมพ์เองได้ในฐานะตัวทานชั้นที่สอง และตอนนั้นติดลบได้** (ส่วนลด/คูปอง)
  (D50/D54 · `docs/adr/0004-surcharge-as-amount.md`) · บิลที่ไม่มี Item ไม่มี Adjustment เลย
...
**แก้ 2026-09-01 (D47): คำสั่งคีย์เวิร์ดในกลุ่มบังคับ mention แล้ว** — ย่อหน้าข้างล่างยังใช้ได้
เต็มตัวสำหรับ `+` ซึ่งไม่ถูกแตะ แต่ `ยอด` `บิล` และคำสั่งอื่นทุกตัวต้องมี `@บิลใหญ่` นำหน้า
เมื่ออยู่ในกลุ่ม · คีย์เวิร์ดเปล่าๆ ในกลุ่มตกเป็นความเงียบ · แชท 1:1 ไม่บังคับเพราะ LINE
ไม่มี mention ที่นั่น
...
| `ยอด` เปล่าๆ ในกลุ่ม | **เงียบ** (D47) | เคยเป็นคำสั่งจนถึง 2026-09-01 · เป็นคำที่คนในกลุ่มพูดกันเองด้วย การตอบจึงเด้งใส่บทสนทนา |
| `ยอด` เปล่าๆ ใน 1:1 | คำสั่ง `balance` | ไม่มี mention ให้ใช้ที่นั่น และไม่มีบทสนทนาของใครให้แทรก |
...
`lib/parser/rules.ts` จึงคืน `adjustmentSatang: 0` เสมอ **โดยตั้งใจ ไม่ใช่ของค้าง** และ
`lib/parser/rules.test.ts` ล็อกไว้แล้ว

Adjustment ในตาราง `expense` ยังจำเป็นจริง แต่เป็นของ **โหมด itemized ซึ่งเป็นงานของ LIFF**
— ตอนหารรายชิ้น ค่าบริการต้องกระจายตามสัดส่วนของแต่ละชิ้น จะกดรวมไว้ในยอดเดียวไม่ได้ ·
**สองอย่างนี้อยู่คนละ surface** ซึ่งเอกสารไม่เคยเขียนไว้ และเป็นสาเหตุที่มันดูเหมือนบั๊ก
ทั้งที่ไม่ใช่ (ชี้แจง 2026-08-30)

**ยืนยัน 2026-09-05 หลังลองกลับคำแล้วกลับมา (D48/D49):** ย่อหน้าข้างบนถูกตั้งคำถามและ
ถูกตัดสินให้กลับคำในเช้าวันนั้น (ไวยากรณ์ `-` ในแชท) แล้ว**กลับมาเป็นแบบเดิมภายในวันเดียวกัน
ก่อนเขียนโค้ดสักบรรทัด** — ราคาที่แพงของ itemized ไม่ใช่ตัวไวยากรณ์ แต่คือการพิมพ์ชื่อคนกิน
ต่อชิ้นด้วยนิ้วโป้งขณะยืนอยู่หน้าเคาน์เตอร์ · การติ๊กชื่อคือสิ่งที่หน้าเว็บทำได้ดีกว่าแชท
อย่างขาดลอย และแชททำแบบติ๊กไม่ได้ด้วยเพราะ LINE ไม่มีการแก้ข้อความที่ส่งไปแล้ว ·
สิ่งที่เปลี่ยนจริงคือ **LIFF ถูกขยับขึ้นมาทำก่อนการทวง** (D49) และ **Surcharge เก็บเป็น
จำนวนเงินไม่ใช่เปอร์เซ็นต์** (D50) · ดู `docs/adr/0003` (กลับคำแล้ว มีเหตุผลอยู่ท้ายไฟล์)
`docs/adr/0004` และ `docs/adr/0005`

**ก่อนมี LIFF ใบเสร็จที่แต่ละคนกินไม่เหมือนกันจดได้ด้วยการยุบตามชุดคนกิน** (เจอตอนยิง M8
จริง 2026-09-04 · รายละเอียดใน `PLAN-M8.md`) — บิลหนึ่งใบไม่ต้องเท่ากับใบเสร็จหนึ่งใบ
รวมรายการที่คนกินชุดเดียวกันเป็นก้อนเดียว แล้วตัดก้อนที่มีแต่คนจ่ายทิ้งเพราะไม่มีใคร
ติดใคร · หนี้ที่ได้เท่ากับ itemized ทุกบาท **สิ่งที่หายไปคือรายการรายชิ้น ไม่ใช่ตัวเลข**
จึงเป็นวิธีที่ใช้ได้จริงไม่ใช่ของชั่วคราวที่ต้องรื้อตอน itemized มา
...
create table expense (
  ...
  adjustment_satang bigint not null default 0,     -- D50/D54 · ติดลบได้เมื่อมีคนพิมพ์เอง
...
create table expense_share (
  ...
  amount_satang  bigint not null,                  -- ยอดสุดท้าย รวม adjustment แล้ว
  unique (expense_id, member_id)
);
-- invariant (ต้องมีเทสต์คุม):
--   Σ amount_satang = total_satang + adjustment_satang      (D50 — ไม่มีการปัดอีกแล้ว)
--
-- กับดักของ D50: ในบิล itemized `total_satang` คือ **ผลรวมรายชิ้น** ไม่ใช่ยอดที่ผู้ใช้พิมพ์
-- ยอดที่พิมพ์ (= ยอดที่จ่ายจริง) คือ `total_satang + adjustment_satang` เสมอ · ทุกที่ที่
-- แสดงคำว่า "ยอดรวม" ต้องใช้ผลบวกนั้น ไม่ใช่ `total_satang` เปล่าๆ · บิลที่ไม่มี Item
-- ได้ `adjustment_satang = 0` ผลบวกจึงเท่าเดิมทุกประการ
--
-- `adjustment_satang` ติดลบได้ (D54) — check ที่ต้องมีคือ `total_satang + adjustment_satang > 0`
-- ไม่ใช่ `adjustment_satang >= 0`
...
เทสต์ที่ต้องมี: invariant `Σ share = total + adjustment` ทุก split mode, เศษ
...
| **M8 `บิล`** | การ์ดรายการบิลตาม D45 · กดแถวดูรายละเอียด · ท้ายการ์ดบอกจำนวนที่เหลือ — **ปุ่มเปิด LIFF (D46) รอ Phase 2** |
| **M9 LIFF + itemized** | ฐาน LIFF (init · verify ID token ฝั่ง server ตาม D15 · route ของหน้าเว็บ) · หน้าจอจดรายชิ้น **ติ๊กชื่อคนกินต่อรายการ** · migration `0003` `adjustment_satang` (D50/D54 — **รันทั้งสอง Supabase project**) · การ์ดรายละเอียดโชว์รายการ (D51) · carousel (D52) — **`itemizedSubtotals` กับ `expense_item` มีอยู่แล้วไม่ต้องเขียนใหม่** · **ไวยากรณ์ในแชทไม่ถูกแตะเลย** |

**แก้แล้ว 2026-09-09 (D58):** การ์ด Draft ในแชท **ถูกวาดใหม่หลังเซฟจากหน้าจอ** — หน้าจอยิง
**text** พร้อมลิงก์ LIFF ของตัวเองกลับเข้าแชท (ชนิดนี้เกิด webhook · Flex ไม่เกิด) แล้วบอทตอบ
การ์ดใบใหม่ด้วย reply token ซึ่งฟรี ไม่กินโควตา 300/เดือน (C2) · การ์ดใบเก่ายังลอยอยู่ในแชท
และกดได้ ซึ่งไม่เป็นไร: `confirmDraft` อ่าน `lines` ใหม่จาก DB เสมอ ยอดที่ลง ledger จึงเป็น
ชุดใหม่ทุกทาง · ส่งไม่ได้ (ยังไม่เปิด scope `chat_message.write` หรือเปิดหน้าจอนอกห้องแชท)
ไม่ใช่ error — ข้อความท้ายหน้าจอเปลี่ยนไปบอกว่ายอดชุดใหม่ลง ledger อยู่ดี

**แก้ 2026-09-08 — เจอจากการยิงของจริงครั้งแรก:** หน้าจอเดาว่า **คนแรกในลิสต์คือคนจ่าย**
เมื่อ draft ไม่มีแถว `isPayer` (บิลที่พิมพ์ชื่อคนโดยไม่ใส่ `รวมฉัน` — คนจ่ายไม่ได้กินด้วย) ·
ค่านั้นถูกเซฟกลับลง draft แล้ว `commitExpense` ยัด `payerMemberId` ให้แถวนั้น
(`lib/repo/confirm.ts`) — **คนนั้นหายจากบิลทั้งคน และคนจ่ายรับหนี้ของเขาไปเงียบๆ** ·
แก้โดย **เอา `payerName` ออกจากสิ่งที่ client ส่งได้** server อ่านคนจ่ายจาก draft ที่เก็บไว้
เท่านั้น (`payerOf`) · เทสต์คุมทั้งสามชั้น: `bill.test.ts` · `save.test.ts` ·
`confirm.db.test.ts`

**Phase 2 — ทวงได้ เคลียร์ได้**
**แก้ 2026-09-05 (D49): LIFF กับ ID token verify ย้ายไปอยู่ M9 แล้ว** — ก้อนนี้จึงเริ่มจาก
ฐานที่มีอยู่ ไม่ได้เริ่มจากศูนย์ · ที่เหลือในก้อนนี้ไม่เปลี่ยน
