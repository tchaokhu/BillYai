...
| D36 | reply ยิงไม่ออก | **ตอบ 200 เสมอเมื่อเข้า handler ได้แล้ว** · 500 สงวนไว้ให้ "เริ่มไม่ได้" (env หาย) | postback ที่ `commitExpense` สำเร็จแล้วแต่ reply พัง ถ้าตอบ 500 LINE จะ retry แล้วเจอว่า draft ถูกลบไปตอน commit → ตอบ "การ์ดเก่าแล้ว พิมพ์ใหม่" ทั้งที่บิลลงเรียบร้อย → คนพิมพ์ซ้ำ = บิลซ้ำจริง · retry มีประโยชน์เฉพาะกับความล้มเหลวที่ retry แล้วผลต่าง ส่วนความล้มเหลวหลังเขียน DB ไปแล้ว retry ทำได้แค่ทำให้เละกว่าเดิม · ฝั่ง draft ก็เหมือนกัน — ไม่มีการ์ด คนพิมพ์ใหม่เอง ของเก่าหมดอายุใน 24 ชม. · log ต้องแยกให้ออกว่า reply พังกี่ครั้ง เพราะเป็นอาการเดียวที่บอกว่า access token หมดอายุหรือโดน rate limit |
