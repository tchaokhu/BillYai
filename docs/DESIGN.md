...
| D48 | ข้อมูลรายชิ้นเข้าทางไหน | **ไวยากรณ์ในแชท ไม่ใช่ LIFF** — บรรทัด `-` ต่อจากหัวบิล `+` | คณิตศาสตร์ ตรวจผลรวม และการเขียน/อ่าน `expense_item` ทำเสร็จมีเทสต์คุมหมดแล้ว ขาดแค่ทางเข้ากับการ์ด · LIFF ยังไม่มีสักบรรทัด รอมันแปลว่ารอทั้ง Phase 2 · **กลับคำ §3 ที่เขียนว่า itemized เป็นของ LIFF** — ดู `docs/adr/0003-itemized-grammar-in-chat.md` |
| D49 | `-` เป็น Trigger ตัวที่สี่ | **ไม่ต้อง @mention ในกลุ่ม** · **ใครในวงก็เพิ่มรายการได้** · มี draft ค้างหลายใบ = ถามด้วยปุ่ม เฉพาะคนพิมพ์ `-` กดได้ | เกณฑ์เดียวกับที่ D47 ยกเว้น `+`: เป็นสัญลักษณ์ ไม่ใช่คำที่คนในกลุ่มพูดกันเอง · ไม่ชน D26 เพราะ D26 คุมที่ปุ่มยืนยัน ไม่ใช่เนื้อ draft และการ์ดถูก render ใหม่กลับเข้ากลุ่มทุกครั้ง · `createDraft` insert เสมอ วงหนึ่งจึงมี draft ค้างพร้อมกันได้หลายใบ ความกำกวมนี้มีมาตั้งแต่ก่อนงานนี้ |
| D50 | Surcharge เก็บยังไง | **จำนวนเงินสตางค์ ไม่ใช่เปอร์เซ็นต์** · เป็นส่วนต่าง ยอดที่พิมพ์ − ผลรวมรายชิ้น · ติดลบ = ปฏิเสธ | `numeric(5,2)` เก็บ `4700/44000 = 10.6818…%` ไม่ลงตัว แล้ว ledger จะได้ 486.99 ทั้งที่ใบเสร็จเขียน 487.00 ซึ่งขัด invariant ใน `CONTEXT.md` · **ต้องมี migration ใหม่ รันทั้งสอง Supabase project** — ดู `docs/adr/0004-surcharge-as-amount.md` |
| D51 | ราคาในบรรทัด `-` | **เลขตัวสุดท้าย** ไม่ใช่ตัวแรกเหมือนหัวบิล | `- โค้ก 2 ขวด 60 aek` ต้องได้ 60 · ชื่อคนที่เป็นตัวเลขล้วนถูก `parseParticipant` ปฏิเสธอยู่แล้ว การสแกนจากขวาจึงปลอดภัยในบรรทัดรายการ · **เอาไปใช้กับหัวบิลไม่ได้** — `+ ค่าห้อง 3000 502` วันนี้เงียบ แต่สแกนจากขวาจะกลายเป็นบิล 502 บาทผิดแบบเงียบ และหัวบิลไม่มีผลรวมรายชิ้นไว้ทาน |
| D52 | ลบรายการออกจาก draft | **ปุ่มต่อแถวบนการ์ด** · postback พา **id สั้นต่อชิ้น ไม่ใช่ index** | การ์ด Draft ใบเก่าลอยอยู่ในแชทตลอดกาลและกดได้เสมอ · index เลื่อนทุกครั้งที่ลบ การกดการ์ดเก่าจะลบผิดชิ้นแบบเงียบ ซึ่งเป็นความผิดชนิดที่ ledger รับไม่ได้ · ทางเลือก "ไม่มีปุ่มลบ ให้พิมพ์ใหม่ทั้งใบ" ถูกปฏิเสธเพราะการพิมพ์ใหม่ 7 รายการเพราะผิดชิ้นเดียวคือความเจ็บที่งานนี้มาแก้ |
| D53 | การ์ดยาวเกิน bubble | **carousel** ทั้งการ์ด Draft และการ์ดรายละเอียด · **ห้ามตัดเป็นบิลใบที่สอง** | ตัดสองบิลทำให้ยอดหัวของแต่ละใบเป็นตัวเลขที่ไม่มีใครพิมพ์ (checksum หาย) ค่าบริการต้องกระจายข้ามสองแถว และถ้ายืนยันสองครั้งแล้วกดใบเดียว **ครึ่งบิลลง ledger** · D44 ยอมลดรูปได้เพราะเป็นแค่การแสดงผล ข้อนี้ไม่ใช่ · เพดาน carousel (~10 bubble / 50 KB) **ยังไม่ได้ยิงจริง ต้องวัดก่อนเชื่อ** |
| D54 | การ์ดของบิล itemized | Draft: รายการเต็มแถว + **รายคนยุบเหลือบรรทัดเดียว** · รายละเอียด: **รายการ + รายคน** ทั้งสองส่วน · **ไม่แยกการ์ดสองแบบ** | บนการ์ด Draft คนกำลังตรวจว่าจดครบไหม รายการจึงต้องอ่านง่ายที่สุด แต่ชื่อทุกคนยังต้องอยู่ครบพร้อมป้าย `(ใหม่)` ตาม D16 · บนการ์ดรายละเอียด รายการตอบ "ใครกินอะไร" รายคนตอบ "ฉันติดเท่าไหร่" ซึ่งเป็นคำถามคนละข้อ · บิลที่ไม่มี Item ไม่ได้ขาดข้อมูล ส่วนรายการจึงหายไปเฉยๆ **ห้ามเขียนว่า "ไม่มีรายการ"** |
| D55 | รายการที่ไม่ระบุชื่อใคร | **ของกลาง หารกับทุกคนในบิล** | ข้าวเปล่า น้ำแข็ง ของกลางคือกรณีปกติของโต๊ะอาหาร ไม่ใช่ edge case · "ทุกคนในบิล" = union ของชื่อที่โผล่ในบรรทัด `-` ทั้งหมด เกณฑ์เดียวกับ D43 · ด่าน `memberIds` ว่าง = error ใน `split.ts:151` ยังอยู่ ทางเข้าเป็นคนเติมชื่อให้ครบก่อนส่งเข้าไป |
...
- **Surcharge หนึ่งตัวต่อบิล** — บิลร้านไทยคือ `รายการ → +10% → +7%` ถ้า itemized
  ไม่มีตัวนี้ ระบบพังทันทีในวันแรก · **เก็บเป็นจำนวนเงินสตางค์ ไม่ใช่เปอร์เซ็นต์**
  และไม่มีใครพิมพ์มันเข้ามา — มันคือส่วนต่างระหว่างยอดที่พิมพ์กับผลรวมรายชิ้น
  (D50 · `docs/adr/0004-surcharge-as-amount.md`) · บิลที่ไม่มี Item ไม่มี Surcharge เลย
...
+ soul bingsu 487                  หัวบิลของใบเสร็จรายชิ้น — ยอดที่จ่ายจริง
- บราวนี่ 30 four nut dear aek     รายการหนึ่งชิ้น หารเท่าเฉพาะสามคนนี้
- ฮันนี่โทสต์ 149 game             ราคาคือ **เลขตัวสุดท้าย** ไม่ใช่ตัวแรก
- ข้าวเปล่า 40                     ไม่ระบุชื่อ = ของกลาง หารกับทุกคนในบิล
- ชาเขียว 60 aek                   พิมพ์ทีหลังเป็นข้อความเดี่ยวก็ได้
...
**แก้ 2026-09-05 (D48/D50): itemized ไม่ใช่ของ LIFF อีกแล้ว และ surcharge ไม่ใช่เปอร์เซ็นต์**
— ย่อหน้าข้างล่างเขียนไว้ตอนที่ยังเชื่อทั้งสองข้อ เก็บไว้เพื่อให้อ่านออกว่าเปลี่ยนอะไรไป
ดู `docs/adr/0003-itemized-grammar-in-chat.md` และ `docs/adr/0004-surcharge-as-amount.md`
...
ที่ยังจริงคือครึ่งเดียว: ค่าบริการต้องกระจายตามสัดส่วน subtotal ของแต่ละคนจริง · ที่ไม่จริง
คือ "อยู่คนละ surface" — **โหมด itemized เข้าทางไวยากรณ์ในแชทตั้งแต่ Phase 1** (D48) และ
ค่าบริการเป็น **ส่วนต่างระหว่างยอดที่พิมพ์กับผลรวมรายชิ้น** ที่ระบบคำนวณเอง เก็บเป็นจำนวนเงิน
ไม่ใช่เปอร์เซ็นต์ (D50) · `surchargePct: 0` ที่ `rules.ts` คืนสำหรับบิลที่ไม่มีรายการยังถูกต้อง
ตามเดิม เพราะบิลที่ไม่มี Item ไม่มี Surcharge เลย
...
create table expense (
  ...
  surcharge_satang bigint not null default 0 check (surcharge_satang >= 0),  -- D50
...
--   Σ amount_satang = total_satang + surcharge_satang       (D50 — ไม่มีการปัดอีกแล้ว)
--
-- กับดักของ D50: ในบิล itemized `total_satang` คือ **ผลรวมรายชิ้น** ไม่ใช่ยอดที่ผู้ใช้พิมพ์
-- ยอดที่พิมพ์ (= ยอดที่จ่ายจริง) คือ `total_satang + surcharge_satang` เสมอ · ทุกที่ที่
-- แสดงคำว่า "ยอดรวม" ต้องใช้ผลบวกนั้น ไม่ใช่ `total_satang` เปล่าๆ · บิลที่ไม่มี Item
-- ได้ `surcharge_satang = 0` ผลบวกจึงเท่าเดิมทุกประการ
...
| **M9 itemized ในแชท** | migration `0003` (D50 — **รันทั้งสอง Supabase project**) · `-` เป็น Trigger (D49) · `items` ใน `ExpenseDraft` · การ์ด Draft แบบรายการ + ปุ่มลบ (D52/D54) · `pendingItem` + ปุ่มเลือกบิล · การ์ดรายละเอียดโชว์รายการ · carousel (D53) — **`itemizedSubtotals` กับการเขียน/อ่าน `expense_item` ไม่ต้องเขียนใหม่ มีอยู่แล้ว** แตะ `split.ts` เฉพาะเรื่อง surcharge |
