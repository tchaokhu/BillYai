...
### 4.3 `chat_message.write` — ตัวที่ทำให้การ์ดใบใหม่โผล่หลังเซฟ (D58)

หลังกด `บันทึกรายการ` หน้าจอยิง **ข้อความ** กลับเข้าแชทด้วย `liff.sendMessages()` แล้วบอท
ตอบการ์ด Draft ใบใหม่ที่ถือยอดชุดใหม่ · เอกสาร LINE ระบุว่าเมธอดนี้ต้องมี scope
**`chat_message.write`** และใช้ได้เฉพาะ LIFF ที่ **เปิดจากห้องแชท** (1:1 · กลุ่ม · ห้อง)
ซึ่งตรงกับ D56 ที่ให้เปิดจากปุ่มบนการ์ดอย่างเดียวอยู่แล้ว

ตรวจที่ `Console → Login channel → แท็บ LIFF → <LIFF app> → Scopes` ว่าติ๊ก
`chat_message.write` ไว้ทั้ง **สองใบ** (`billyai-dev` และใบของ production ตอนปล่อยจริง)

| อาการ | สาเหตุ |
|---|---|
| เซฟแล้วขึ้น `บันทึกแล้ว — ส่งการ์ดใบใหม่เข้าแชทให้แล้ว` แต่ในแชทไม่มีการ์ดมา | ข้อความออกไปแล้วแต่บอทอ่านไม่ออก — `NEXT_PUBLIC_LIFF_ID` ที่ฝังในหน้าจอ **คนละใบ**กับที่ฝั่ง server อ่าน (คนละ environment ของ Vercel หรือยังไม่ได้ Redeploy หลังแก้) |
| เซฟแล้วขึ้น `กลับไปกดยืนยันบนการ์ดในแชท` (ข้อความแบบเก่า) | ส่งไม่ออก — ยังไม่ติ๊ก `chat_message.write` หรือหน้าจอถูกเปิดนอกห้องแชท · **ไม่ใช่ error** บิลถูกเซฟแล้วและกดยืนยันบนการ์ดใบเก่าได้ ยอดที่ลง ledger เป็นชุดใหม่ |
| บอทตอบ `การ์ดใบนี้ใช้ไม่ได้แล้ว` ทันทีหลังเซฟ | draft หมดอายุ (24 ชม.) หรือถูกกดยืนยันไปแล้วระหว่างที่หน้าจอเปิดค้าง |
| มีคนแปะลิงก์เก่าซ้ำแล้วการ์ดโผล่อีกใบ | ตั้งใจ — ใครในวงเดียวกันแปะลิงก์ก็ได้การ์ดใบนั้นซ้ำ · ข้ามวงไม่ได้ (`findDraftInScope`) |
...
**ลบแล้ว 2026-09-08** — หน้า `/liff/spike` เป็นของชั่วคราวของ Phase 0 · S2 บันทึกผลไว้ใน
`SPIKE-PHASE0.md` เรียบร้อยแล้ว และหน้าจอจริงอยู่ที่ `/liff` · **LIFF app ใบเดิมที่ชี้มาที่นี่
ต้องเปลี่ยน Endpoint URL เป็น `/liff`** ไม่งั้นกดแล้วได้ 404
